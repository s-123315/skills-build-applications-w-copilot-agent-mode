...
const getCollection = async (model: Model<any>, fallback: any[]) => {
  try {
...
app.get('/api/users/', async (_req, res) => {
  const users = await getCollection(User, fallbackUsers);
...
app.get('/api/activities/', async (_req, res) => {
  const activities = await getCollection(Activity, fallbackActivities);
